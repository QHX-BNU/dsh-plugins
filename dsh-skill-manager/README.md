...
- **@ 输入选择器（两级路由）**：对话输入框输入 `@` 先弹出「技能 / 工作区文件」两个分类（与"子智能体/命令"并列），点击分类后再显示对应内容：
  - **技能**：按名称/描述/适用场景模糊搜索，选中后插入**真实文本** `/skill-name `——光标可自由移动/编辑（可进入文字中间）；输入框内经宿主 text-ref 扫描 + 插件隐藏 lexicon 源装饰为带背景的引用样式。发送文本即 `/skill-name`，服务端 `SKILL_GESTURE` 识别（等价于直接输入 `/skill-name`）；
  - **工作区文件**：按文件名/路径搜索当前会话工作区（cwd）下的文件（依赖 `dsh-code-panel` 的 `/code-panel/api/search`，未安装时显示提示），选中后以**引用框（chip）**插入输入框（label 为「文件 · 相对路径」），发送时序列化为 `@相对路径`，模型可据此直接读取工作区文件（文件路径含 `.`/`/`，宿主 text-ref 扫描仅支持 `[\w-]+` 名称，故文件保持 occurrence chip）；
  - 技能/文件引用都带背景色与圆角边框（技能=蓝、文件=绿），宽度自适应内容、超长 ellipsis 截断（悬停 title 可看完整）；点击分类后 `@` 后已输入的查询词自动延续到第二级；菜单关闭（选中/Esc）后，下一次 `@` 重新从分类层开始。
...
- **@ 工作区文件（两级菜单 + 结构引用）**：`@` 菜单先显示「选择」分类层（技能 / 工作区文件），点击分类后通过宿主 launcher（`controller.toggleSource`）打开只含该分类的单分组菜单，`@` 后已输入的查询词延续；三个 @ 源（选择/技能/工作区文件）始终注册，按会话内当前阶段返回候选（非当前阶段返回空数组，空分组不渲染）。文件分组从 `sessions` 服务取当前会话 `cwd`，调用 `dsh-code-panel` 的 `/code-panel/api/search` 递归搜索（文件名/路径子串匹配、前缀优先）。**技能**选中返回 `{ text: "/skill-name " }`（真实文本，光标可自由定位），并注册 trigger `/` 的隐藏 lexicon 源（无候选、不渲染分组）让宿主 text-ref 扫描把 `/skill-name` 装饰为带背景的引用样式（插件覆盖 `[data-decoration="text-ref"]` 加背景、负 margin 保持对齐）。**文件**选中返回 `{ insert: { source, ref, label, clipboardText } }`——宿主输入机器 mint 为 occurrence chip（占位符 `￼` + 背景色引用框），提交时经 `codec.serialize` 展开为 `@相对路径`（与宿主 subagent 引用 `@name` 惯例一致）。两个插件通过 `dsh-code-panel:probe` 事件做能力探测。
